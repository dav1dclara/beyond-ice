<template>
  <div class="navigation-controls">
    <button @click="$emit('zoom')" class="zoom-to-extent-button" title="Zoom to extent">
...
const addNavigationControl = () => {
  if (!props.map || navControl) return;
  
  navControl = new mapboxgl.NavigationControl();
  props.map.addControl(navControl);
};

const removeNavigationControl = () => {
  if (props.map && navControl) {
    props.map.removeControl(navControl);
    navControl = null;
  }
};
...
.navigation-controls {
  position: absolute;
  top: 56px; /* Position below NavigationControl (10px top + 36px height + 10px gap) */
  right: 10px;
  z-index: 1000;
}
